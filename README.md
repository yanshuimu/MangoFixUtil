...
MangoFixUtil是对[MangoFix](https://github.com/YPLiang19/Mango)进行了简单的封装，该库在OC项目中实战已经近2年多，经过多次迭代，比较成熟。但需要搭配[补丁管理后台](https://patchhub.top/mangofix/login)一起使用，后台由作者开发维护，目前有50+个已上架AppStore的应用在使用，且有日活1w+的应用，欢迎小伙伴们使用。

声明：该项目不管现在还是以后都不会有任何形式的收费，也不需要打赏，更不会停服，服务器一直在正常运行，安心使用。

如果你觉得不错，可以给个小星星支持，在此表示感谢^_^。
